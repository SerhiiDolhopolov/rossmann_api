...
from app.schemas import (
    ...
    ProductAdminSchema,
    CityProductSchema,
    CityProductAdminSchema,
    CityProductAddSchema,
    CityProductUpdateSchema,
    CityProductPatchSchema,
)
from app.config import TAG_ADMIN
from app.kafka.producer import get_kafka_producer, upsert_product_to_local_db
...
async def get_products_by_city(
    ...
    result = [
        CityProductSchema(
            product=ProductSchema.model_validate(product, from_attributes=True),
...
async def get_products_page_by_city(
    city_id: int,
    ...
    products = await get_products_by_city(
        city_id=city_id,
...
async def get_all_products_by_city(
    city_id,
    skip: int = Query(0, ge=0),
    limit: int = Query(100, gt=0, le=100),
    db=Depends(get_db),
    ...
    result = [
        CityProductAdminSchema(
            product=ProductAdminSchema.model_validate(product, from_attributes=True),
            price=city_product.price,
            discount=city_product.discount,
...
async def add_product_to_city(
    ...
    await upsert_product_to_local_db(
        producer=producer,
        product_id=new_city_product.product_id,
        name=new_city_product.product.name,
        description=new_city_product.product.description,
        barcode=new_city_product.product.barcode,
        category_id=new_city_product.product.category_id,
        price=new_city_product.price,
        discount=new_city_product.discount,
        is_deleted=new_city_product.is_deleted,
    )

    result = CityProductAdminSchema(
        product=ProductAdminSchema.model_validate(new_city_product.product, from_attributes=True),
        price=new_city_product.price,
        discount=new_city_product.discount,
        is_deleted=new_city_product.is_deleted,
    )
...
async def replace_product_in_city(
    city_id: int,
    product_id: int,
    ...
    return await update_product_in_city_method(
        city_id=city_id,
        product_id=product_id,
...
async def update_product_in_city(
    city_id: int,
    product_id: int,
...
async def update_product_in_city_method(
    city_id: int,
    product_id: int,
    city_product,
    db,
    ...
    check_city(city_id, db)
    check_product(product_id, db)
    existing_city_product = (
        db.query(CityProduct)
        .filter(
            CityProduct.city_id == city_id,
    ...
    await upsert_product_to_local_db(
        producer=producer,
        product_id=existing_city_product.product_id,
