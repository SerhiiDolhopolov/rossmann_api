...
@router.get(
    "/{user_id:int}/shopping_cart",
    summary="Return dict of shopping card. {product_id: count}",
    response_model=dict,
)
...
@router.patch(
    "/{user_id:int}/shopping_cart",
    summary=(
        "Update a product in the user's shopping cart. If quantity=0, remove the product. "
        "Update cart lifetime to 24 hours."
    ),
    response_model=dict,
)
