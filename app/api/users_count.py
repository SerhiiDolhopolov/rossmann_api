...
@router_admin.get("/by_ip", summary='Return approximate quantity of users by IP.', response_model=int)
...
@router_admin.post("/add_ip", summary='Add IP to count')
async def add_ip(ip: IPvAnyAddress = Query(example='192.168.123.130'), redis=Depends(get_redis)):
    """
...
@router_admin.delete("/reset_ip", summary='Reset IP count')
async def reset_ip_count(redis=Depends(get_redis)):
    ...
    return { 'status': 'success', 
             'message': f'IP count reset successfully' }
    
@router_admin.get("/by_email", summary='Return approximate quantity of users by email.', response_model=int)
...
@router_admin.post("/add_email", summary='Add email to count')
async def add_email(email: EmailStr = Query(example='example@example.com'), redis=Depends(get_redis)):
    """
        Add an IP address to the HyperLogLog count.
    """
    try:
...
@router_admin.delete("/reset_email", summary='Reset email count')
async def reset_email_count(redis=Depends(get_redis)):
    ...
    return { 'status': 'success', 
             'message': f'Email count reset successfully' }